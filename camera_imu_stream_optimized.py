#!/usr/bin/env python3
"""
Optimized low-latency web stream for IMX500 camera + BNO085 IMU visualization
...
STREAM_CONFIG = {
    'resolution': (480, 360),  # Reduced resolution for lower latency
    'jpeg_quality': 70,        # Lower quality for faster encoding
    'target_fps': 25,          # Slightly lower FPS for stability
    'max_frame_age': 0.2,      # Drop frames older than 200ms
    'frame_buffer_size': 2     # Keep only 2 latest frames
}
...
def initialize_camera():
    """Initialize the IMX500 camera with optimized settings"""
    global camera
    ...
    try:
        ...
        # Configure camera with optimized settings for low latency
        config = camera.create_preview_configuration(
            main={"size": STREAM_CONFIG['resolution']},  # Smaller resolution
            raw={"size": (2028, 1520)},
            encode="main",
        ...
        camera.start()
        print(f"✓ Camera started at {STREAM_CONFIG['resolution']} resolution")
        return True
        ...
        try:
            ...
            camera.configure(camera.create_preview_configuration(
                main={"size": STREAM_CONFIG['resolution'], "format": "RGB888"},
                buffer_count=2
            ))
            camera.start()
...
def capture_frames():
    ...
    while True:
        try:
            ...
            # Add minimal overlay (reduced text for faster processing)
            timestamp = time.strftime("%H:%M:%S")
            cv2.putText(frame, timestamp, (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 
                       0.6, (0, 255, 0), 1)
            
            # Add stream info
            info = f"IMX500+IMU Low-Latency"
            ...
            if ret:
                ...
                # Update frame buffer with thread safety (always keep latest frames)
                with frame_lock:
...
def get_latest_frame():
    """Get the most recent frame, dropping old ones"""
    global frame_buffer, frame_lock, frame_stats
    ...
    with frame_lock:
        ...
        while frame_buffer:
            ...
            if age > STREAM_CONFIG['max_frame_age']:
                dropped_frame = frame_buffer.popleft()
        ...
        if frame_buffer:
            ...
            frame_stats['avg_age'] = current_time - latest_frame['timestamp']
            return latest_frame
...
def generate_frames():
    ...
    while True:
        ...
        if frame_data is None:
            time.sleep(0.001)  # Very short sleep if no frame available
            continue
        
        frame_count += 1
        
        # Add frame statistics every 100 frames
...
@app.route('/')
def index():
    """Main page with optimized video stream and IMU visualization"""
    return '''
<!DOCTYPE html>
<html>
<head>
    <title>Low-Latency Robot Camera + IMU Stream</title>
...
<body>
    <div class="container">
        <div class="header">
            <h1>🚀 Low-Latency Robot Camera + IMU Stream</h1>
            <p>Optimized for minimal delay - Real-time camera and IMU visualization</p>
        </div>
        
        <div class="performance-info">
            <strong>Optimization Features:</strong> 
            Frame Dropping • Reduced Resolution • Minimal Buffering • Fast JPEG Encoding
        </div>
        ...
        <div class="content">
            <div class="video-section">
                ...
                <div class="video-container" style="position: relative;">
                    <img src="/video_feed" alt="Low-Latency Camera Stream" id="video-stream">
                    <div class="latency-indicator" id="latency">
                        Latency: --ms
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        videoElement.onload = function() {
            const latency = performance.now() - streamStartTime;
            document.getElementById('latency').textContent = `Latency: ${latency.toFixed(0)}ms`;
            streamStartTime = performance.now(); // Reset for next frame
        };
        
        // IMU data polling with higher frequency for responsiveness
        ...
        window.addEventListener('load', () => {
            ...
            // Start IMU data polling with higher frequency
            updateIMUData();
            setInterval(updateIMUData, 50); // 20 Hz update rate for better responsiveness
        });
...
@app.route('/status')
def status():
    """Status endpoint with performance info"""
    return {
        'status': 'running',
...
def main():
    """Main function"""
    print("=" * 70)
    print("🚀 Low-Latency Robot Camera + IMU Web Stream Server")
    print("=" * 70)
    print(f"📊 Stream Configuration:")
    print(f"   Resolution: {STREAM_CONFIG['resolution']}")
    print(f"   JPEG Quality: {STREAM_CONFIG['jpeg_quality']}%")
    print(f"   Target FPS: {STREAM_CONFIG['target_fps']}")
    print(f"   Max Frame Age: {STREAM_CONFIG['max_frame_age']}s")
    print("-" * 70)
    ...
    print("🎬 Starting optimized background threads...")
    
    # Camera capture thread with high priority
    ...
    print("✅ Low-latency stream ready!")
    print(f"🌐 Access at: http://localhost:5000")
    print("📱 Or from network: http://YOUR_PI_IP:5000")
    print("🎯 Features: Frame dropping, minimal buffering, optimized encoding")
    print("🛑 Press Ctrl+C to stop")
    print("=" * 70)
    
    try:
        # Configure Flask for low latency
        app.run(
            host='0.0.0.0', 
            port=5000, 
            debug=False, 
