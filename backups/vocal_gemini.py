...
class AudioHandler:
    ...
    def __init__(self):
        ...
        # queues (50 packets ≈ 1 s @ 20 ms)
        self.audio_out_q: asyncio.Queue[bytes] = asyncio.Queue(maxsize=50)
        self.audio_in_q: asyncio.Queue[bytes] = asyncio.Queue()

        # logging WAVs
        self.sent_wf = wave.open("sent_audio.wav", "wb")
        self.sent_wf.setnchannels(CHANNELS)
        ...
        self.recv_wf.setnchannels(CHANNELS)
    ...
    def _find_input_device(self) -> int:
        for idx in range(self.pya.get_device_count()):
            ...
            if info.get("maxInputChannels", 0) and ("MPOW" in info["name"] or "HC6" in info["name"]):
                # print(f"Selected input device {idx}: {info['name']}")
                return idx
        info = self.pya.get_default_input_device_info()
        # print(f"Using default input device {info['index']}: {info['name']}")
        return info["index"]

    def _find_output_device(self) -> int:
        for idx in range(self.pya.get_device_count()):
            info = self.pya.get_device_info_by_index(idx)
            if info.get("maxOutputChannels", 0) and any(k in info["name"].lower() for k in ("pulse", "default")):
                # print(f"Selected output device {idx}: {info['name']}")
                return idx
        info = self.pya.get_default_output_device_info()
        # print(f"Using default output device {info['index']}: {info['name']}")
        return info["index"]
        ...
        while True:
            ...
            async for resp in turn:
                ...
                if resp.data:
                    ...
                    self.audio_in_q.put_nowait(resp.data)
                if resp.text:
                    print(f"\nGemini: {resp.text}")
                    pass
